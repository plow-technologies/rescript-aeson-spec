...
function decode_ssample_unsafe(decode, json) {
  return /* record */[
          /* seed */Aeson_decode.field("seed", Aeson_decode.$$float, json),
...
function decode_ssample(decode, json) {
  var exit = 0;
  var v;
  try {
    v = decode_ssample_unsafe(decode, json);
    exit = 1;
  }
  catch (raw_exn){
    var exn = Js_exn.internalToOCamlException(raw_exn);
...
function encode_ssample(encode, sample) {
...
function server_test(decode, encode, url, value_of_type) {
...
function server_roundtrip(decode, encode, name_of_type, url, value_of_type) {
  ...
  describe("AesonSpec.server_roundtrip: " + name_of_type, (function () {
          return Jest.testPromise("send to and receive from server: " + encodedString, (function () {
                        return server_test(decode, encode, url, value_of_type);
                      }));
        }));
  return /* () */0;
}
    ...
    describe("golden test for: " + name_of_type, (function () {
            return List.iter((function (sample) {
                          ...
                          return Jest.testPromise("server", (function () {
                                        return server_test(decode, encode, url, Aeson_decode.unwrapResult(Curry._1(decode, sample)));
                                      }));
                        }), $$Array.to_list(sample[/* samples */1]));
          }));
    return /* () */0;
...
function ggolden(decode, encode, name_of_type, url, json_file) {
  var json = JSON.parse(Fs.readFileSync(json_file, "utf8"));
  var match = decode_ssample(decode, json);
  if (match.tag) {
    var error = match[0];
    describe("", (function () {
            return Jest.test("", (function () {
    ...
    var sample = match[0];
    describe("golden test for: " + name_of_type, (function () {
            var encoded = encode_ssample(encode, sample);
            Jest.test("golden file", (function () {
