...
function ggolden(decode, encode, name_of_type, _, json_file) {
  var json = JSON.parse(Fs.readFileSync(json_file, "utf8"));
  var match = decode_sample(json);
  if (match.tag) {
    var error = match[0];
    ...
    describe("golden test for: " + name_of_type, (function () {
            return Jest.test("", (function () {
                          var decoded = List.map((function (a) {
                                  return Aeson_decode.unwrapResult(Curry._1(decode, a));
                                }), $$Array.to_list(sample[/* samples */1]));
                          return Jest.Expect[/* toEqual */12]($$Array.to_list(sample[/* samples */1]))(Jest.Expect[/* expect */0](List.map(encode, decoded)));
                        }));
          }));
    return /* () */0;
  }
