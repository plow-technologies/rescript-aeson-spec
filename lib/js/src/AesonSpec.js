...
var InternalJest = require("./InternalJest.js");
...
function jsonRoundtripSpec(decode, encode, json) {
  ...
  return InternalJest.Expect[/* toEqual */12](/* Ok */Block.__(0, [json]), InternalJest.Expect[/* expect */0](resultMap(encode, rDecoded)));
}

function sampleJsonRoundtripSpec(decode, encode, json) {
  var rDecoded = decodeSample(decode, json);
  if (rDecoded.tag) {
    return InternalJest.fail("Unable to decode golden file.");
  } else {
    ...
    if (a !== undefined && b !== undefined) {
      ...
      var xs = List.map((function (param) {
              return InternalJest.Expect[/* toEqual */12](param[1], InternalJest.Expect[/* expect */0](param[0]));
            }), z);
      var os = getFirstFail(xs);
      if (os !== undefined) {
        return os;
      } else {
        return InternalJest.pass;
      }
    } else {
      return InternalJest.fail("Did not find key 'samples'.");
    }
  }
}

function valueRoundtripSpec(decode, encode, value) {
  return InternalJest.Expect[/* toEqual */12](/* Ok */Block.__(0, [value]), InternalJest.Expect[/* expect */0](Curry._1(decode, Curry._1(encode, value))));
}

function goldenSpec(decode, encode, name_of_type, json_file) {
  return InternalJest.describe("AesonSpec.goldenSpec: " + (name_of_type + (" from file '" + (json_file + "'"))), (function (param) {
                var json = JSON.parse(Fs.readFileSync(json_file, "utf8"));
                return InternalJest.test("decode then encode: " + JSON.stringify(json), (function (param) {
...
function sampleGoldenSpec(decode, encode, name_of_type, json_file) {
  return InternalJest.describe("AesonSpec.sampleGoldenSpec: " + (name_of_type + (" from file '" + (json_file + "'"))), (function (param) {
                var json = JSON.parse(Fs.readFileSync(json_file, "utf8"));
                return InternalJest.test("decode then encode json_file", (function (param) {
